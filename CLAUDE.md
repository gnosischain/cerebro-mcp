...
Four personas own data planes that are OUTSIDE the semantic registry / dbt catalog — dbt discovery (`find`, `search_models`, `discover_models`) returns only noise for their topics, so they skip it by design:
...
- **`pool_liquidity_analyst`** — Gnosis Chain DEX pool liquidity (Uniswap v3 + Swapr v3/Algebra concentrated-liquidity state and ticks; Balancer v2/v3 reserves only) over the `rpc_state_indexer` publication-verified plane (chain_id 100; already in `CURATED_RAW_DATABASES`). Reads ONLY that database — never joins dbt models, so no USD and no symbols beyond `v_token_metadata_current`; prices are token1-raw per token0-raw in "raw units" unless both decimals resolve. `config_registry` is the only relation that takes `FINAL`; every `v_*_published` view is read without it and the raw `pool_*` tables are never read (their key includes `attempt_id`, so FINAL still returns duplicate attempts); as-of dates resolve from SERVED publications (`v_publications_current`, bounded to candidate days — a raw `census_publications` row is not a served snapshot) and every view scan is pruned with `snapshot_date = (SELECT as_of FROM asof)`. Only pools above the indexer's active threshold have ticks — the rest carry `cl_below_active_threshold` and are state-only (NULL, never 0). Visuals default to `open_pools_explorer`; USD / volume questions hand off to `defi_analyst`.

Gate accommodation (enforced in code): `describe_table` on a **curated raw database** (`CURATED_RAW_DATABASES` in `config.py`: `cow_db`, `governance_db`, `rpc_log_indexer`, `rpc_state_indexer`, plus the RPC scratch DB when scans are enabled) counts as both discovery and lineage for the chart gates — one `find(mode="chart")` + one in-domain `describe_table` opens `quick_chart`/`generate_charts`; a full report needs three in-domain describes. dbt-plane behavior is unchanged.
