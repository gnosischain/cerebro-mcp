// Deferred dataset-group loader shared by v2 mini-apps (CoW Explorer,
// Governance Explorer, ...).
//
// A section apply (`load_<app>_section`) only loads the section's `core`
// group; every other group streams in afterwards through an app-only
// `load_<app>_datasets` tool. This hook drives that streaming:
//
...
export function useGroupLoader(callTool: GroupCallTool, toolName: string): GroupLoader {
    ...
    (viewId: string, section: string, groups: string[], scopeId: string) => {
      for (const group of groups) {
        ...
        callTool(toolName, {
          ...
          .catch((err) => {
            console.error(`[${toolName}] group ${section}.${group} failed`, err);
