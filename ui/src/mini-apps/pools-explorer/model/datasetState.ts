// Pure per-dataset display-state mapper (copied from governance/model/
// datasetState.ts — a shared module would be free to shift under both apps).
// One place understands the server's warning-code contract so every panel
...
//   no_indexed_data / zero rows    -> "empty"
//   otherwise                      -> "ready"
//
// `groupLoaded` is the `loaded_groups["section.group"]` value; the `"partial"`
// sentinel means the group finished loading but at least one of its datasets
// shipped a failure stub — per-dataset codes still decide which dataset
// failed, so "partial" alone never forces a failed panel.
...
export function datasetDisplayState(
  ...
  if (codes.includes("no_indexed_data") || codes.includes("no_data") || rowCount === 0) return "empty";
  return "ready";
}
