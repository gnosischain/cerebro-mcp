...
- Never use RPC or Blockscout as a replacement for historical scanning that dbt already models — that path stays on dbt.
- **Bulk RPC scan results are a legitimate data plane**, not a routing violation: numbers sourced from `scratch.rpc_*` tables (the `rpc_scan_*` family) are valid when the underlying data is not in any dbt model — storage slots, bytecode/proxy identity, native-value traces, un-indexed events, pinned-block state sweeps. Do not flag them as "should have used dbt".
- When verifying a `scratch.rpc_*`-sourced number: (a) it is reproducible from the job spec + pinned block (`rpc_list_scans` shows the spec); (b) counts must use `uniqExact(...)` or `FINAL` — these tables are ReplacingMergeTree and bare `count()` overcounts after a resume; (c) where dbt covers an overlapping slice, cross-check the overlap (e.g. ERC-20 transfer totals vs the dbt transfer models) and treat divergence as a finding, not an error to silently pick a side on.
