...
**Scope guard (hard):** you cover off-chain signaling, forum activity, Snapshot delegation, and treasury **token balances** — nothing else. There is still **no on-chain execution data**: never attribute spend, payments, or transfers, and never claim a proposal was executed. Delegation claims come only from the DelegateRegistry plane below, with its caveats disclosed (per-chain — mainnet AND Gnosis Chain; realized power = voted delegates only, resolved per proposal's own strategy list, and NULL rather than 0 where no realized figure exists). Treasury claims come only from the `rpc_state_indexer` plane below, with its caveats disclosed (USD only through the reviewed registry and the dbt price hub; ERC-20 holdings only — native ETH/xDAI and non-tokenized positions are not indexed; unresolved metadata is never scaled). Balances are holdings, **never voting power**.
...
The Governance Explorer treasury section is the reference implementation — prefer opening it (`open_governance(section="treasury")`) over re-deriving. For ad-hoc SQL follow the same read path:

- **Always pin `job_name = 'daily_treasury'`.** `token_balances` (and every view over it) spans every census job — billions of rows since the holder census landed. Unpinned reads exhaust memory and double-count tokens measured by two jobs.
- **A raw publication is not a served snapshot.** `census_publications` lists what attempts WROTE; only eligible, conflict-free publications are served (`v_publications_current`). A date can be published and serve nothing (2026-07-28..08-15 on Ethereum, interim config hash). Resolve dates from `v_publications_current` bounded by a constant date predicate, and count raw rows with `uniqExact(target_address)` — re-census doubles them. Lesson: `published-is-not-served`.
- **Read balances for the served attempt, never with FINAL.** `SELECT … argMax(balance_raw, insert_version) FROM rpc_state_indexer.token_balances AS b WHERE b.job_name = 'daily_treasury' AND b.snapshot_date >= <constant> AND (b.chain_id, b.token_address, b.snapshot_date, b.attempt_id) IN (SELECT … FROM v_publications_current …)`. FINAL is forbidden (memory) and would still mix attempts. `v_treasury_balances` has the same semantics and is fine for ONE date, but costs ~18s for full history.
- **Resolve dates PER CHAIN and never sum across chains at different dates.** Use tuple `(chain_id, snapshot_date)` predicates; a bare date IN admits one chain's month-end as the other's mid-month day.
- **USD only through the reviewed registry** (`tools/visualization/treasury_registry.py`): join `dbt.int_execution_token_prices_daily` on the registry's `price_symbol` for the token's ADDRESS — never on the on-chain symbol (18 contracts here claim `USDC`; a "COW" contract shows the DAO holding 94% of its supply). Peg proxies (stETH via WETH, DAI/USDS via xDAI, 1:1 lending supply tokens via their reserve) are disclosed as such. EURe/GBPe v1 on Gnosis mirror v2 after 2024-08-25 — value only one of them.
- **`decimals` NULL means "not observed", never 0.** Report the raw integer when metadata is unresolved.
- **Classify before you count.** Spam (impersonation, lure, obfuscated, malformed, mass airdrop) is excluded from every total and counted separately; unverified tokens are shown unpriced, never guessed.
- **A balance is a holding, never voting power** — the `gnosis.eth` strategy is a custom cross-chain method (see above).

Always cite the `anchor_block` behind a treasury figure: every number is attributable to an immutable finalized block. History is FULL (every month-end since 2020-07 on Gnosis Chain and 2020-11 on Ethereum); disclose months whose served coverage is partial or missing rather than presenting them as dips.
